    ...
    def prepareGradingAB(self):      #aka Make AB-Folders for Lesson
        ...
        if self.status != 4:
            ...
            while ( bookCount > 0 ) :
                ...
                elif bookCount in range ( folderSizeMargin, folderSize ):
                    try:    #with transaction.atomic():
                        folderCount += 1
                ...
                elif bookCount in range ( 0, folderSizeMargin ):
                    try:    #with transaction.atomic():
                        # get last folder (aka folderCount)
                        thisFolderSize = folderSize + bookCount
                        Folder.filter(LessonID = self, no=folderCount).update(books = thisFolderSize).save()
